...
include_once('functions/icefit-mce/editor-buttons.php'); // Editor Buttons
include_once('functions/visual-shortcodes/visual-shortcodes.php'); // Visual Shortcodes
...
function icefit_styles() {
	...
	foreach($fonts_to_use as $font_to_load):
		if ("" != $font_to_load && !in_array($font_to_load, $websafe_fonts))
			wp_enqueue_style( $font_to_load, "$protocol://fonts.googleapis.com/css?family=" . str_replace(" ", "+", $font_to_load) . ':400italic,700italic,400,700', array(), null );
	endforeach;
}
...
function icefit_generate_dynamiccss($id) {
	...
	// Boxed layout
	if ('Boxed' == $icefit_options['layout'] ):
		
		switch ( $icefit_options['background_image_repeat'] ):
		...
		switch ( $icefit_options['background_image_position'] ):
		...
		switch ( $icefit_options['background_image_attachment'] ):
		...
		$background_color = $icefit_options['background_color'];
		$background_image = $icefit_options['background_image'];
		
		$custom_css .= "body {";
		if ("" != $icefit_options['background_image'] && "null.png" != substr($icefit_options['background_image'], -8) )
			$custom_css .= "background-image: url('".$background_image."');";
		$custom_css .= "background-color: ".$icefit_options['background_color'].";
...
#sticky-header #logo img { height: 30px; width: auto; }";
	...
	// Header settings
	$header_layout = $icefit_options['header_layout'];
	...
	$custom_css .= '#header {
	padding-top: '.$icefit_options['header_padding_top'].'px;
	padding-bottom: '.$icefit_options['header_padding_bottom'].'px; }';
	
	if ( $icefit_options['header_title'] == 'Display Title' )
		$custom_css .= '#logo .site-title {
	font-family: '.$icefit_options['header_title_font_family'].';
	color: '.$icefit_options['header_title_font_color'].';
	font-size: '.$icefit_options['header_title_font_size'].'px;
	}';

	if ( $icefit_options['header_layout'] == 'Logo left + Tagline' )
		$custom_css .= '#tagline {
			font-family: '.$icefit_options['header_tagline_font_family'].';
			color: '.$icefit_options['header_tagline_font_color'].';
			font-size: '.$icefit_options['header_tagline_font_size'].'px;
			}';


	// Navbar
	$custom_css .= '#navbar ul li * { font-family:'.$icefit_options['menu_font'].'; font-size: '.$icefit_options['menu_font_size'].'px; }';
	
	if ( $icefit_options['menu_layout'] == 'Center No Search' )
	...
	// Headings font
	$custom_css .= "#page-container h1, #page-container h2, #page-container h3, #page-container h4, #page-container h5, #page-container h6 { font-family: ".$icefit_options['headings_font'].', "Lucida Grande", "Lucida Sans Unicode", Helvetica, Arial, Verdana, sans-serif; }';

	// Headings Styling
	$custom_css .= 'h1, h1 a, h1 a:visited {
		font-size: '.$icefit_options['h1_font_size'].'px;
		color: '.$icefit_options['h1_font_color'].'; }
	h2, h2 a, h2 a:visited {
		font-size: '.$icefit_options['h2_font_size'].'px;
		color: '.$icefit_options['h2_font_color'].'; }
	h3, h3 a, h3 a:visited {
		font-size: '.$icefit_options['h3_font_size'].'px;
		color: '.$icefit_options['h3_font_color'].'; }
	h4, h4 a, h4 a:visited {
		font-size: '.$icefit_options['h4_font_size'].'px;
		color: '.$icefit_options['h4_font_color'].'; }
	h5, h5 a, h5 a:visited {
		font-size: '.$icefit_options['h5_font_size'].'px;
		color: '.$icefit_options['h5_font_color'].'; }
	h6, h6 a, h6 a:visited {
		font-size: '.$icefit_options['h6_font_size'].'px;
		color: '.$icefit_options['h6_font_color'].'; }
	
	h1.page-title {
		font-size: '.$icefit_options['page_title_font_size'].'px;
		color: '.$icefit_options['page_title_font_color'].'; }

	#page-container h3.entry-title a,
	#page-container h3.entry-title a:visited,
	.post-content h1.entry-title {
		font-size: '.$icefit_options['entry_title_font_size'].'px;
		color: '.$icefit_options['entry_title_font_color'].'; }';

	 // Content font
	 $custom_css .= '#page-container > div, #page-container > span, #page-container > p, #page-container > table, #page-container > form, #page-container > figure, #page-container > ul, #page-container > ol, #page-container > dl, #page-container > address, #page-container > pre, #footer .container ul > *, #sidebar .textwidget, #sidebar p, #sidebar .widget_rss ul li, #footer p, #footer .container .widget_rss ul li { font-family: '.$icefit_options['content_font'].';
	 font-size: '.$icefit_options['content_font_size'].'px;  }';

	 $custom_css .= '#sub-footer { font-family: '.$icefit_options['content_font'].'; }';
	 
	 // Sidebar Widgets
	 $custom_css .= '#sidebar h3.widget-title {
	 	font-family: '.$icefit_options['sidebar_widget_title_font'].';
	 	font-size: '.$icefit_options['sidebar_widget_title_font_size'].'px; }';

	 // Footer Widgets
	 $custom_css .= '#footer .widget-title {
	 	font-family: '.$icefit_options['footer_widget_title_font'].';
	 	font-size: '.$icefit_options['footer_widget_title_font_size'].'px;
	 	color: '.$icefit_options['footer_widget_title_font_color'].'; }';


	// User defined additional custom CSS
	$custom_css .= $icefit_options['custom_css'];
